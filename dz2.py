...
headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.105 YaBrowser/21.3.3.234 Yowser/2.5 Safari/537.36'}
...
def seach_hh(query):
    ...
    base = []
    while True:
        url = f'https://hh.ru/search/vacancy?L_is_autosearch=false&clusters=true&area=1&search_field=name&enable_snippets=true&salary=&st=searchVacancy&text={query}&page={i}'
        page = bs(requests.get(url, headers=headers).text, 'html.parser')
        describe_vacancy = page.find_all('div', attrs={'class': 'vacancy-serp-item__row_header'})
        temp = list(map(lambda x: {'Зарплата': x.contents[1].text.replace('\xa0', ''), 'Ссылка': x.a['href'],
        ...
        for d in temp:
            ...
            d['Ссылка'] = d['Ссылка'][:d['Ссылка'].find('?')]
...
def seach_superjob(query):
    ...
    while True:
        ...
        temp = list(map(lambda x: {'Зарплата': x.contents[1].text.replace('\xa0', ''), 'Ссылка': url[:23] + x.a['herf'],
                                   'Наименование вакансии': x.a.text,
                                   'Ресурс': url[:23]}, describe_vacancy))

        for d in temp:
            if d['Зарплата'] == 'По договоренности':
               d['Зарплата'] = None
            elif '-' in d['Зраплата']:
                    temp_str = d['Зарплата'].split('-')
                    temp_cur = temp_str[1].split('руб/мес')
                    if len(temp_cur) == 1:
                ...
                temp_cur = d['Зарплата'].split('руб/мес')
                if len(temp_cur) == 1:
                ...
                if temp_cur[0][:2] != 'от' and temp_cur[0][:2] != 'до':
                    ...
                    d['Зарплата'] = {'минимальная': int(temp_cur[0][2:] if temp_cur[0][2:] == 'от' else None),
                                     'максимальная': int(temp_cur[0] if temp_cur[0][2:] == 'до' else None),
                                     'валюта': 'руб/мес'}
